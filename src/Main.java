...
public class Main {
    public static void main(String[] args) {
        ...
        // Test the GP model
        DataLoader.Dataset testDataset = (DataLoader.Dataset) testData;
        double[] predictions = gp.predict(testData);
        
        // Calculate test accuracy
        int correct = 0;
        for (int i = 0; i < predictions.length; i++) {
            if (predictions[i] == testDataset.labels[i]) {
                correct++;
            }
        }
        ...
        // Print results
        System.out.println("\n---- Results ----");
        System.out.printf("Genetic Programming (seed=%d)\n", seed);
        System.out.printf("Test Accuracy: %.4f\n", accuracy);
        System.out.println("Best program: " + gp.getBestProgram());
